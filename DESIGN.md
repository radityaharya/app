...
The single-font decision is the brand. There is no display face, no body sans, no italic alternative, and no fallback to a proportional font anywhere — even the legal copyright row uses Berkeley Mono at 14px. This is the most aggressive typographic restraint of any site in the marketing-tools category: Commuter's identity is "the marketing page is a man page."

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.display-xl}` | 38px | 700 | 1.5 | 0 | Hero headline ("Personal helper. One toolkit.") |
| `{typography.heading-md}` | 16px | 700 | 1.5 | 0 | Section label ("What is Commuter?", "FAQ", "Built for privacy first") |
...
- **ASCII block-pixel wordmark** — the Commuter brand name rendered as a 5-row block of monospaced character cells, used in the primary nav and as the centerpiece of the hero TUI mockup.
- **Hero TUI mockup** — full-bleed `{colors.surface-dark}` rectangle containing a faux terminal interface: ASCII wordmark, a `tui-prompt-row` showing a primary command line, and `tab switch module` / `ctrl-p commands` keybinding hints in `{colors.ash}` at the bottom edge.
- **Chart tiles** — three thin-line ASCII charts inside the home page's personal-helper / all-in-one-toolkit stat block, with abstract dotted/sparse-line plots in `{colors.body}` against the cream canvas. Captions sit beneath in `{typography.caption-md}` (`Fig 1. Daily saves`, `Fig 2. Tasks cleared`, `Fig 3. Time back`).
...
**`button-primary`** — the universal Commuter CTA
...
- Renders as a bare `**Heading**` line above a 1px `{colors.hairline}` rule with no chip background — but the way the text reads ("[+]", "[x]", `What is Commuter?`) makes it function as a label component.
...
- Contains the literal `curl -fsSL https://Commuter.ai/install | bash` command with a small copy-icon at the right edge. Sits below the install-method tab strip.
...
- Contents (top → bottom): ASCII block-pixel "Commuter" wordmark centered in `{colors.on-dark}`; a `{component.tui-prompt-row}` showing a primary command line with module selector text; an `tab switch module  ctrl-p commands` keybinding hint row at the bottom in `{colors.ash}`.
...
- Renders an inline command (`Open  Today  Reminders  Commuter`) with a leading vertical pipe and the active module styled as a bracketed token.
...
- Each row begins with a `[+]` / `[-]` / `[x]` ASCII marker followed by a bold label and a regular description: e.g., `[+] One surface    Commands and shortcuts without hopping between apps`. The bracket marker is part of the text content, not a separate icon.
...
- Layout (desktop): block-pixel ASCII Commuter wordmark at left (~120×24px), nav links cluster center-right ("GitHub [150K] · Docs · Zen · Go · Enterprise"), `{component.button-primary}` "Download" CTA at the far right with a small download glyph.
...
- Don't render the Commuter wordmark as a vector logo. It is always block-pixel ASCII.
...
- **Mobile screenshots not captured** — responsive behavior synthesizes Commuter's mobile pattern (hamburger drawer, single-column, footer accordion) from desktop evidence and the breakpoint stack.
- **Hover states not documented** by system policy.
- **In-product TUI screenshots** beyond the marketing hero mockup are not in the captured set; the actual `Commuter` terminal interface (full keybindings, panels, status bar) is not documented here.
